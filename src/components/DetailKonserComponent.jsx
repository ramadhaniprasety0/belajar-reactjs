...
const DetailKonserComponent = ({ konserDetail }) => {
  ...
  // Definisikan paymentLink di sini, pastikan konserDetail.id tersedia
  const paymentLink = konserDetail.id ? `/bayar-konser/${konserDetail.id}` : '#';
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <i key={`full-${i}`} className="bi bi-star-fill star-icon"></i>
      );
    }
    if (halfStar) {
      ...
      <div className="detail-konser-content-wrapper">
        ...
        <Row className="mb-5 main-info-section align-items-start g-md-5">
            ...
            <div className="price-rating-container d-flex align-items-center my-3">
              ...
              <div className="rating-info">
                ...
                <span className="rating-details-text d-block">
                  ({konserDetail.ratingValue || konserDetail.ratingStars}{" "}
                  Bintang) • {konserDetail.reviewCount}
                </span>
              </div>
            </div>
          </Col>
          ...
          <Col md={3} lg={3} className="konser-sidebar d-flex flex-column">
            ...
            <div className="info-box date-time-box mb-4">
              {" "}
              {/* Tambahkan mb-4 untuk jarak antar box */}
              <p className="info-box-main-text">{konserDetail.dateTime.date}</p>
              <p className="info-box-sub-text mb-0">
                {konserDetail.dateTime.time}
              </p>
            </div>
            ...
            <div className="info-box location-box mb-4">
              ...
              <div className="d-flex align-items-center location-content">
                {" "}
                {/* align-items-center untuk ikon besar */}
                <i className="bi bi-geo-alt-fill location-icon"></i>{" "}
                {/* me-2 dihapus, diatur di CSS */}
                <div>
            ...
            <Button
              as={Link} // Render Button sebagai komponen Link
              to={paymentLink} // Tentukan tujuan link secara dinamis
              variant="primary"
        ...
        <section className="detail-section map-section">
          ...
          <div className="map-image-container">
            <Image
              src={
                konserDetail.mapImage ||
                "https://via.placeholder.com/1200x350/f0f0f0/cccccc?text=Peta+Lokasi+Konser"
              }
            ...
            {konserDetail.livePhotos.map((photo, index) => (
              <Col key={index} xs={6} sm={4} md={3} lg={3}>
                <div className="live-photo-wrapper">
                  {" "}
                  {/* Ukuran wrapper tetap dari CSS sebelumnya */}
                  <Image
                    src={
                      photo ||
                      `https://via.placeholder.com/250x150/f0f0f0/cccccc?text=Live+Photo+${
                        index + 1
                      }`
                    }
